...
export function associatedWithLabel(document: {
  applicantLinks?: {
    application: {
      ...
      applicant: { legalFirstName: string; legalLastName: string };
    };
  }[];
  ...
  const parts = [
    ...(document.applicantLinks ?? []).map(
      (link) =>
        `Applicant: ${formatPersonName(link.application.applicant.legalFirstName, link.application.applicant.legalLastName)} · ${link.application.trackingNumber}`,
    ),
