...
export function isWeakBetterAuthSecret(secret: string) {
  ...
  return WEAK_SECRETS.has(secret.toLowerCase());
}
...
/**
 * Build may use a placeholder so Next can compile without a live secret.
 * Runtime must never sign sessions with a missing or weak secret.
 */
